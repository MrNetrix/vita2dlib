...
static inline void set_palette_program()
{
	sceGxmSetVertexProgram(_vita2d_context, _vita2d_paletteVertexProgram);
	sceGxmSetFragmentProgram(_vita2d_context, _vita2d_paletteFragmentProgram);
}
...
void vita2d_draw_palette_texture(const vita2d_texture *texture, const vita2d_texture *palette, float x, float y, float x_scale, float y_scale)
{
    vita2d_texture_vertex *vertices = (vita2d_texture_vertex *) vita2d_pool_memalign(
        4 * sizeof(vita2d_texture_vertex), // 4 vertices
        sizeof(vita2d_texture_vertex));
    ...
    sceGxmSetUniformDataF(vertexDefaultBuffer, _vita2d_paletteWvpParam, 0, 16, _vita2d_ortho_matrix);

    // Set the texture to the TEXUNIT0
    sceGxmSetFragmentTexture(_vita2d_context, 0, &texture->gxm_tex);
    sceGxmSetFragmentTexture(_vita2d_context, 1, &palette->gxm_tex);
